...
export const useGetPatientVisits = (
  patientId: string
): UseQueryResult<VisitWithRelations[], Error> => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.get<VisitWithRelations[]>(
    ...
    mutationFn: async (data: CreateVisitDto | FormData) => {
      ...
      if (USE_MOCK_DATA) {
        ...
        const mockResult = await mockVisitsAPI.createVisit(
          visitData as typeof visitData & {
            chief_complaint: string;
            diagnosis: string;
            vitals: { weight: number };
          }
        );
...
export const useUpdateVisit = (): UseMutationResult<
  Visit,
  Error,
  { id: number; data: Partial<VisitFormData> }
> => {
  const queryClient = useQueryClient();
...
export const useGetRecentVisits = (
  limit: number = 10
): UseQueryResult<VisitWithRelations[], Error> => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.get<VisitWithRelations[]>(
        `/doctor/visits/recent?limit=${limit}`
      );
      return response.data;
    },
    staleTime: 2 * 60 * 1000,
 ...
 * ```
 */
export const useGetAllVisits = (params?: {
  page?: number;
  limit?: number;
}): UseQueryResult<PaginatedVisitsResponse, Error> => {
  const { user } = useAuthStore();
  const isAdmin = user?.role === Role.ADMIN;
  ...
  return useQuery<PaginatedVisitsResponse>({
    ...
    queryFn: async (): Promise<PaginatedVisitsResponse> => {
      ...
      if (isAdmin) {
        const response = await adminApi.getClinicVisits({
          page: params?.page || 1,
...
export const useGetAllPatients = (): UseQueryResult<unknown[], Error> => {
  ...
  return useQuery<unknown[]>({
    ...
    queryFn: async (): Promise<unknown[]> => {
      ...
      // ADMIN uses clinic-scoped endpoints, DOCTOR uses doctorApi
      if (isAdmin) {
        const response = await adminApi.getClinicPatients({
          page: 1,
          limit: 10000,
        });
