...
export function AdminPatientProfile({
  ...
  const getScanTypeLabel = (
    ...
    const typeMap: Record<string, string> = {
      '0': tCommon('scanTypes.mri'),
      '1': tCommon('scanTypes.ct'),
      '2': tCommon('scanTypes.xray'),
      '3': tCommon('scanTypes.ultrasound'),
      '4': tCommon('scanTypes.petct'),
      '5': tCommon('scanTypes.mammography'),
    };
    return typeMap[String(typeValue)] || String(typeValue);
  };

  const { data: visitsData, isLoading: visitsLoading } =
    useAdminGetPatientVisits(patient.id);
  const { data: medsData, isLoading: medsLoading } =
    useAdminGetPatientMedications(patient.id);
  const { data: labsData, isLoading: labsLoading } = useAdminGetPatientLabs(
    patient.id
  );
  const { data: scansData, isLoading: scansLoading } = useAdminGetPatientScans(
    patient.id
  );
        ...
        <div className="overflow-x-auto scrollbar-hide">
          <TabsList className="inline-flex w-auto min-w-full md:grid md:w-full md:grid-cols-4">
            <TabsTrigger value="visits" className="text-xs sm:text-sm">
              {t('visits')}{' '}
              {visitsData?.clinics
                ? `(${visitsData.clinics.reduce((acc, c) => acc + c.visits.length, 0)})`
                : ''}
            </TabsTrigger>
            <TabsTrigger value="medications" className="text-xs sm:text-sm">
              {t('medications')}{' '}
              {medsData?.medications ? `(${medsData.medications.length})` : ''}
            </TabsTrigger>
            ...
            <TabsTrigger value="scans" className="text-xs sm:text-sm">
              {t('scans')}{' '}
              {scansData?.scans ? `(${scansData.scans.length})` : ''}
            </TabsTrigger>
          </TabsList>
        </div>
            ...
            visitsData.clinics.map((clinic) => (
              <div key={clinic.id} className="border rounded-lg p-4 space-y-3">
                ...
                {clinic.visits.map((visit, idx) => (
                  <div
                    key={idx}
                    className="pl-4 border-l-2 border-medical-primary/20 space-y-1"
                  >
                    <p className="text-sm font-medium">{visit.diagnoses}</p>
                    <p className="text-xs text-muted-foreground">
                      {visit.doctor.name} ({visit.doctor.speciality}) •{' '}
          ...
          {medsLoading ? (
            ...
            medsData.medications.map((med, idx) => (
              <div key={idx} className="border rounded-lg p-4 space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  {t('dosage')}: {med.dosage}x/{t('day')} • {t('period')}:{' '}
                  {med.period} {t('days')}
                </p>
                ...
                {med.commentsAudioUrl && (
                  <AudioLink url={med.commentsAudioUrl} />
                )}
              </div>
            ))
          ) : (
            ...
            labsData.labs.map((lab, idx) => (
              <div key={idx} className="border rounded-lg p-4 space-y-2">
                ...
                {lab.commentsAudioUrl && (
                  <AudioLink url={lab.commentsAudioUrl} />
                )}
              </div>
            ))
          ) : (
              ...
              <div key={idx} className="border rounded-lg p-4 space-y-2">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <span className="text-xs text-medical-primary">
                ...
                {scan.commentsAudioUrl && (
                  <AudioLink url={scan.commentsAudioUrl} />
                )}
              </div>
            ))
          ) : (
