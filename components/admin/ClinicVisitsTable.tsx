...
import { useAdminGetClinicVisits } from '@/lib/api/queries/useAdmin';

export function ClinicVisitsTable() {
  const t = useTranslations('admin');
  const [page, setPage] = useState(1);
  const limit = 10;

  const { data, isLoading, error } = useAdminGetClinicVisits({ page, limit });
                ...
                <TableRow key={visit.id}>
                  ...
                  <TableCell>
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{visit.doctor.speciality}</p>
                    </div>
                  </TableCell>
