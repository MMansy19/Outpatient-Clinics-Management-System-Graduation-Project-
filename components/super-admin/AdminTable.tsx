...
export function AdminTable() {
  ...
  const loadAdmins = async () => {
    try {
      setLoading(true);
      const data = await superAdminApi.getAdmins({ page, limit });
    ...
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">{t('loadingAdmins')}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-4">
            ...
            >
              ...
              <div className="space-y-2 pt-3 border-t border-gray-100 dark:border-gray-700">
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-gray-600 dark:text-gray-400">
                    {admin.user.socialSecurityNumber}
                  </span>
                </div>
              </div>
              ...
              {admins.length === 0 ? (
                ...
                admins.map((admin) => (
                  ...
                  >
                    ...
                    <TableCell>{admin.user.socialSecurityNumber}</TableCell>
                    <TableCell>
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button variant="ghost" size="sm" className="text-red-600 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-2">
        <p className="text-xs sm:text-sm text-muted-foreground text-center sm:text-left">
          {t('showing')} {admins.length > 0 ? (page - 1) * limit + 1 : 0} -{' '}
