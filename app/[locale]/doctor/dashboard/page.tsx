...
export default function DoctorDashboard({ params }: DoctorDashboardProps) {
  ...
  const { data: allVisits, isLoading: loadingAllVisits } = useGetAllVisits({
    page: 1,
    limit: 50,
  });
  ...
  const calculateStats = () => {
    ...
    const todaysPatients = Array.isArray(allPatients)
      ...
      : allPatients &&
          typeof allPatients === 'object' &&
          'items' in allPatients &&
          Array.isArray((allPatients as any).items)
    ...
    const pendingVisits =
      allVisits?.items?.filter((visit: any) => {
        ...
        {currentView === 'profile' && selectedPatientId && (
          <div className="space-y-4">
            ...
            <PatientProfile
              ...
              patient={(Array.isArray(allPatients)
                ? allPatients
                : (allPatients as any)?.items
              )?.find(
                (p: any) =>
